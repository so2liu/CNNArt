...
def compute_MSSIM_loss(dHyper, x_ref, decoded_ref2ref, decoded_art2ref):
    ...
    mu_x = Lambda(lambda x: K.sum(w * x, axis=(1,2,3)), output_shape=(None,))(x_ref)

    mu_y_art2ref = Lambda(lambda x: K.sum(w * x, axis=(1,2,3)), output_shape=(None,))(decoded_art2ref)

    mu_y_ref2ref = Lambda(lambda x: K.sum(w * x, axis=(1, 2, 3)), output_shape=(None,))(decoded_ref2ref)

    sigma_x2 = Lambda(lambda x: K.sum(w * K.square(x), axis=(1,2,3)) - K.square(mu_x), output_shape=(None,))(x_ref)

    sigma_y2_art2ref = Lambda(lambda x: K.sum(w * K.square(x), axis=(1,2,3)) - K.square(mu_x), output_shape=(None,))(decoded_art2ref)

    sigma_y2_ref2ref = Lambda(lambda x: K.sum(w * K.square(x), axis=(1, 2, 3)) - K.square(mu_x), output_shape=(None,))(decoded_ref2ref)

    sigma_xy_art2ref = Lambda(lambda x: K.sum(w * x[0] * x[1], axis=(1,2,3)) - mu_x * mu_y_art2ref, output_shape=(None,)) \
        ([(x_ref), (decoded_art2ref)])

    sigma_xy_ref2ref = Lambda(lambda x: K.sum(w * x[0] * x[1], axis=(1, 2, 3)) - mu_x * mu_y_ref2ref, output_shape=(None,)) \
    ...
    loss_ref2ref = 1 - K.sum(l_ref2ref * cs_ref2ref)
    loss_art2ref = 1 - K.sum(l_art2ref * cs_art2ref)
