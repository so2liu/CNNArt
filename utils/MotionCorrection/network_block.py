...
def decode(input, patchSize, conv_1, conv_2, conv_3, conv_4):
    if list(patchSize) == [48, 48]:
        ...
        # output shape: (512, 10, 10)
        output1 = fCreateConv2DTranspose_ResBlock(filters=512)(reshape)
        output2 = add([output1, conv_4])
        # output shape: (256, 20, 20)
        output3 = fCreateConv2DTranspose_ResBlock(filters=256)(output2)
        output4 = add([output3, conv_3])
        # output shape: (128, 40, 40)
        output5 = fCreateConv2DTranspose_ResBlock(filters=128)(output4)
        output6 = add([output5, conv_2])
        # output shape: (64, 80, 80)
        output7 = fCreateConv2DTranspose_ResBlock(filters=64)(output6)
        output8 = add([output7, conv_1])
        # output shape: (64, 80, 80)
        output9 = fCreateConv2DTranspose_ResBlock(filters=64, kernel_size=(7, 7), strides=(1, 1))(output8)
        # output shape: (1, 80, 80)
        output10 = Conv2DTranspose(filters=1, kernel_size=1, strides=1, padding='same', activation='tanh')(output9)
